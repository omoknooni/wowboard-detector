...
이미지 파일명에는 가급적 특수문자 넣는것을 피한다. 폰 카메라로 활영된 이미지의 기본 파일명에서 수정하지 않아도 OK.

## 진행 프로세스

보드 이미지 촬영 후, **이미지 사이즈를 조절해 주어야한다**. 기본적으로 폰 카메라로 촬영하는 경우 사이즈가 약 3000x3000으로 사이즈가 크게 찍히는데, 이런 경우 학습 과정에서 이미지를 일부 영역만 인식하게 되어 학습이 제대로 되지 못한다.
따라서, 첨부된 image_resizer.py를 통해 1024x1024 사이즈로 자르는 작업이 필요하다.  

1. **촬영 담당자**는 이미지를 반드시 **'photoUpload'** 폴더 내 알맞은 위치에 업로드한 뒤 resize 담당자에게 알려줄 것. 

2. **resize 담당자**는 이미지의 사이즈를 조절한 후 원본을 original 폴더로 이동한 후 사이즈 조절된 이미지를 resized 폴더로 이동시킨다. 이후 라벨링 담당자에게 알려줄 것.

3. **라벨링 담당자**는 resized 폴더 내 이미지를 각자 로컬에 다운받아 라벨링 작업을 진행한 후 결과물(png, xml)을 **labelled**폴더 내 알맞은 위치에 업로드한다.

최종적인 모델 학습에는 labelled 폴더 내 데이터가 사용되니 업로드 시 폴더를 혼동하지 않도록 유의!
...
> **라벨링 시 라벨 이름 고정!**
> 1. stickynote : 포스트잇
> 2. wowboard-front : 와우보드 전면 객체
> 3. wowboard-back : 와우보드 후면 객체
